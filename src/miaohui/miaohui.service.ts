...
export class MiaohuiService {
  ...
  async addFriendByPhone(phone: string, helloMsg: string): Promise<{ ok: boolean; code: number; raw?: any }> {
    const token = this.config.get('MIAOHUI_GROUP_TOKEN');
    if (!token) return { ok: false, code: -99, raw: '缺 MIAOHUI_GROUP_TOKEN' };
    try {
      const { data } = await axios.post(
        `${this.base}/addFriend/send`,
        {
          token,
          phoneNum: phone,
          helloMsg,
          userId: this.config.get('MIAOHUI_BOT_USERID', 'jiahongjia'),
